...
struct {
  ...
  struct rb_tree rbtree; // Red-Black Tree for process scheduling
} ptable;
...
// scheduler(void)
// {
//   struct proc *p, *most_prioritized_proc;
//   struct cpu *c = mycpu();
//   c->proc = 0;
...
//     // Initiates variables for priority mechanism
//     most_prioritized_proc = 0;
//     int max_priority = -1;

//     // find the highest prioritize runnable process
//     for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
//       if(p->state == RUNNABLE && p->priority > max_priority){
//         max_priority = p->priority;
//         most_prioritized_proc = p;
//       }
//     }
...
//     if(most_prioritized_proc){
//       p = most_prioritized_proc;
//       c->proc = p;
//       switchuvm(p);
//       p->state = RUNNING;
